...
|B4|レース/出走表/成績の正規化（races, entries, results, changes）|設計完了（実装は未）|RaceList/DebaTable/RaceMarkTable のパース契約とDB反映（upsert順序・優先順位）を確定。`source_shared/28_race_entry_result_parser_design.md` と `02_database_design.md`, `source_shared/25_database_definition.md`, `source_shared/20_parser_normalization_contracts.md` を更新|
...
|ID|内容|状態|備考|
|---|---|---|---|
|C1|DBスキーマ初期実装（PostgreSQL）|設計完了|`02_database_design.md` と `source_shared/25_database_definition.md` を元にマイグレーション|
...
参照: `02_database_design.md`, `source_shared/25_database_definition.md`, `source_shared/29_db_migration_and_bootstrap.md`
...
- すべてのHTTP取得について `raw_fetch_logs` を必ず記録（成功/失敗を問わない）
