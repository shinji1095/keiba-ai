...
- 定期実行の設定更新: `POST /scrape/schedule`
  - `enabled`: boolean
...
- `RaceList?k_raceDate=...&k_babaCode=...`
  ...
  - 各レースへの導線（出馬表・成績・オッズ）
  - 注意: RaceList の発走時刻（start_time）は一次情報だが、HTML構造や表示揺れにより欠損し得る。
    - 欠損時は DebaTable のヘッダ（post_time）を補助情報として利用し、後続の取得判断（特に RefundMoneyList の最終取得タイミング）に反映する。
...
※ t_minus_20m / t_minus_10m / t_minus_5m / t_minus_1m はディアクティベート（取得対象外）。
...
推奨（最小）
- 最終レース後（**最終発走時刻 + 90分 程度**）に 1回取得
  - 最終発走時刻の決定:
    - 基本: RaceList の start_time（開催場内の最大）
    - 補助: DebaTable の post_time（開催場内の最大）で **last_start_dt を更新**し、RaceList の start_time 欠損時でも最終取得が行われるようにする
