...
def test_plan_scheduler_saves_state_every_5_tasks(tmp_path: Path) -> None:
    ...
    race_date = "2026-01-14"
    plan_path = control_dir / f"scrape_plan_{race_date}.json"
    generated_at = "2026-01-14T08:00:00+09:00"
    items = [_plan_item(race_date=race_date, scheduled_at=f"2026-01-14T08:00:{i:02d}+09:00") for i in range(6)]
    _write_plan(plan_path, race_date=race_date, generated_at=generated_at, items=items)
