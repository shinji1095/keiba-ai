# DBスキーマ提案（PostgreSQL / 正規化前提）

更新履歴
- 2026-01-03: 着別成績（全/左/右/場/距）を別テーブル化し参照キーで管理。数値化方針（着別成績分解、タイム秒、誕生日月日、通過順配列）を追記。

取得元（定義根拠）
- 地方競馬情報サイト. "出馬表の見方（着別成績・最高タイム）". https://www.keiba.go.jp/Howtoread/theracecard.html (accessed 2026-01-03).

## 正規化の意図
- **重複排除**: 馬・人物・血統などの実体は複数レースで繰り返されるため、独立テーブル化して参照で管理する。
- **更新容易性**: オッズ等の変動情報はスナップショット化（captured_at）し、履歴と差分取得に耐える。
- **学習データ適合**: 数値として扱えるデータは可能な限り数値型で保持する。
  - 着別成績 `1-5-4-8` は「1着-2着-3着-4着以下」の回数なので 4 つの整数に分解して保存する。
    - 注意: DebaTable には「騎手」欄に *騎乗成績*（当該馬に騎乗した際の騎手の成績）も `1-5-4-8` 形式で表示されるため、`perf_*`（全/左/右/場/距）と混同しない。
  - タイム `1:32.9` は `time_sec=92.9` のように秒へ変換して保存する。
  - 生年月日（年不明の月日 `MM.DD`）は birth_month/birth_day として数値化して保存できる。
  - コーナー通過順 `4-4-3-2` は INT[]（例: '{4,4,3,2}'）として格納できる（raw も併記推奨）。
  - 減量記号（★/▲/△/◇/☆）は `apprentice_allowance_symbol` と `apprentice_allowance_kg` に分解して保持する（負担重量の表示値は減量後のため、基準重量は派生値として扱う）。
- **条件別成績の独立**: perf_total / perf_dirt_left / perf_dirt_right / perf_track / perf_distance を別テーブル化し、race_entry から参照する。
...
-- affiliation は NULL 可だが一意キーに含めるため、NULL を sentinel 値へ寄せて UNIQUE を担保する
CREATE UNIQUE INDEX uq_person_name_role_affiliation
...
-- オッズ（単勝・複勝）スナップショット
CREATE TABLE odds_tanfuku_snapshot (
  race_id TEXT NOT NULL REFERENCES race(race_id),
  horse_no SMALLINT NOT NULL,
...
CREATE TABLE weight_allowance_symbol (
...
-- オッズ（枠連）スナップショット
CREATE TABLE odds_wakuren_snapshot (
  race_id TEXT NOT NULL REFERENCES race(race_id),
...
CREATE TABLE odds_umaren_snapshot (
  race_id TEXT NOT NULL REFERENCES race(race_id),
...
CREATE TABLE odds_umatan_snapshot (
  race_id TEXT NOT NULL REFERENCES race(race_id),
  horse_no_1 SMALLINT NOT NULL,
  horse_no_2 SMALLINT NOT NULL,
  captured_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE odds_wide_snapshot (
  race_id TEXT NOT NULL REFERENCES race(race_id),
  horse_no_1 SMALLINT NOT NULL,
  horse_no_2 SMALLINT NOT NULL,
  captured_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
## ETLメモ（参照キー運用）
- `horse` / `person` / `perf_*` / `best_time` は UPSERT し、`RETURNING ..._id` で参照キーを取得して `race_entry` に格納する。
- 学習用途では、`race_entry` を中心に JOIN するか、特徴量用の非正規化ビュー/テーブルを別途作成する。
